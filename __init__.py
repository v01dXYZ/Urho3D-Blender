...
from .Urho3DRenderer import Urho3DRenderEngine,get_panels
...
#         print("dim:%s|%s"%(region.width,region.height))
...
#         print("CUSTOMDA")
#         self.pixels = None
#         pixels = [1.0, 0.2, 0.1, 1.0] * width * height
...
#     def draw(self):
#         print("CUSTOMDATA:draw")              
#         #pixels = [random.random(), 0.2, 0.1, 1.0] * width * height
#         #screenshotPath = bpy.context.scene.urho_exportsettings.screenshotPath+"/Screenshot.png"
#         #if self.imgPath!=screenshotPath:
#         #    try:
#         #        self.img = bpy.data.images.load(screenshotPath)
#         #        self.imgPath=screenshotPath
#         #    except:
#         #        print("could not load image:%s" % screenshotPath)
#         #        return
...
#         tex = bgl.Buffer(bgl.GL_BYTE, len(self.pixels), self.pixels)
...
#         bgl.glActiveTexture(bgl.GL_TEXTURE0)
#         bgl.glBindTexture(bgl.GL_TEXTURE_2D, self.texture[0])
#         bgl.glTexImage2D(bgl.GL_TEXTURE_2D, 0, bgl.GL_RGB8, width, height, 0, bgl.GL_RGB, bgl.GL_BYTE, tex)
#         bgl.glTexParameteri(bgl.GL_TEXTURE_2D, bgl.GL_TEXTURE_MIN_FILTER, bgl.GL_LINEAR)
#         bgl.glTexParameteri(bgl.GL_TEXTURE_2D, bgl.GL_TEXTURE_MAG_FILTER, bgl.GL_LINEAR)
#         bgl.glBindTexture(bgl.GL_TEXTURE_2D, 0)
...
#             if panel.__name_Urho3DRenderEngine_ not in exclude_panels:
#                 panels.append(panel)
...
def OpDeleteLodSet(self,context):
    id = bpy.context.active_object.lodsetID
    idx = getLodSetWithID(id,True)
    # remove the lodset
    lodset = bpy.data.worlds[0].lodsets.remove(idx)
    # remove all reference to this lodset from all objects
    for obj in bpy.data.objects:
        if obj.lodsetID == id:
            obj.lodsetID = -1
            print("Removed lodset from object %s" % obj.name)
...
class UrhoExportSettings(bpy.types.PropertyGroup):
    ...
    screenshotPath : bpy.props.StringProperty(
                    name="ScreenshotPath",
...
def register():
    ...
    bpy.utils.register_class(Urho3DRenderEngine)
...
def unregister():
    ...
    bpy.utils.unregister_class(Urho3DCustomRenderEngine)
